...
/**
 * Load the Clawdrive grant signing private key from environment.
 */
async function getSigningKey(): Promise<CryptoKey> {
  const keyStr = process.env.CLAWDRIVE_SIGNING_PRIVATE_KEY;
  if (!keyStr) {
    throw new Error("CLAWDRIVE_SIGNING_PRIVATE_KEY is not configured");
  }
  return importPrivateKey(keyStr);
}

/**
 * Load the Clawdrive grant signing public key from environment.
 */
export async function getSigningPublicKey(): Promise<CryptoKey> {
  const keyStr = process.env.CLAWDRIVE_SIGNING_PUBLIC_KEY;
  if (!keyStr) {
    throw new Error("CLAWDRIVE_SIGNING_PUBLIC_KEY is not configured");
  }
 ...
 * 1. JWT signature valid (verified by jose against Clawdrive's public key)
