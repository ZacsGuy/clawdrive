...
export function registerAuthCommands(program: Command): void {
  ...
  auth
    .command("login")
    .description("Log in to Clawdrive via browser-based authentication")
    .action(async () => {
      const config = readConfig();
      const baseUrl = config.apiBaseUrl ?? "https://api.clawdrive.dev";
  ...
  auth
    ...
    .action(() => {
      ...
      if (config.authToken) {
        ...
        console.log(`Run ${chalk.cyan("clawdrive auth login")} to authenticate.`);
      }
    });
}
