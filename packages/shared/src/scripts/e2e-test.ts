/**
 * Full end-to-end integration test against a live Clawdrive deployment.
 *
 * Usage:
 *   CLAWDRIVE_URL=https://www.clawdrive.sh \
...
const BASE = process.env.CLAWDRIVE_URL ?? "https://www.clawdrive.sh";
...
async function getOrCreateTestUser(): Promise<string> {
  ...
  const user = (await clerkAPI("/users", "POST", {
    email_address: [`clawdrive-test-${Date.now()}@clawdrive.sh`],
    password: "ClawdriveTest1234",
  })) as { id: string };
  return user.id;
}
